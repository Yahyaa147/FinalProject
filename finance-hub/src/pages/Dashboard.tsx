...
const Dashboard = () => {
  const { 
    ...
    // Use news store for reading statistics
  const { getReadingStats, favoriteArticles } = useNewsStore();
  ...
  const portfolioStats = [
    ...
    },
    {
      ...
      title: 'Articles Read',
      value: readingStats.todayArticles.toString(),
      icon: BookOpen,
      color: 'text-green-600',
      bgColor: 'bg-green-100',
      subtitle: `${favoriteArticles.length} favorites`
    }
  ];
  return (
        ...
        <div className="mb-8">          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-6 flex items-center transition-colors duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {portfolioStats.map((stat, index) => {
              ...
              return (
                <div key={index} className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200/50 dark:border-gray-700/50 hover:shadow-md transition-all duration-300 group">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className={`p-4 rounded-xl ${stat.bgColor} dark:bg-opacity-20 group-hover:scale-110 transition-transform duration-300`}>
                        <Icon className={`h-6 w-6 ${stat.color} dark:text-opacity-90`} />
                      </div>
                      <div className="ml-4">
                        <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1 transition-colors duration-300">{stat.title}</p>                        <p className={`text-2xl font-bold ${stat.color} dark:text-opacity-90 transition-colors duration-300`}>
                          {stat.value}
                        </p>
                        {stat.subtitle && (
                          <p className={`text-sm font-medium ${stat.color} dark:text-opacity-80 flex items-center mt-1 transition-colors duration-300`}>
                            {stat.subtitle}
